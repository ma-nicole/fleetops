...
export default function FeedbackPage() {
  ...
  const payment = CustomerDataFlowService.getPaymentForCurrentBooking();
  const latestFeedback = CustomerDataFlowService.getFeedback().find((item) => item.bookingId === booking?.id);

  const submit = () => {
    setError("");
    const result = CustomerDataFlowService.saveFeedback(message || "Great service", rating);
        ...
        <section style={{ background: "white", border: "1px solid #E8E8E8", borderRadius: "10px", padding: "1rem", display: "grid", gap: "0.5rem" }}>
          ...
          {latestFeedback?.receiptMessage && (
            <p style={{ margin: 0, color: "#4B5563" }}>
              <strong>Email Sent:</strong> {latestFeedback.receiptMessage}
            </p>
          )}
        </section>
      </div>
